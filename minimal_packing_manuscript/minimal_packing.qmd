...
Further, they concluded that self-similar fractal distribution was a good model for pore structure, but not necessarily grain size.
...
Using these definitions, algorithms are presented for both an approximate and a rigorous solution for the MDM.
The results show a broad range of MDM magnitudes over several GSDs of interest in geotechnical engineering.
The MDM algorithms are implemented in an open-source Python module, available on GitHub.
...
This is analogous to the limitation of scope in ASTM D6913 to grain sizes passing a 75-mm sieve, ensuring the cumulative GSD always reaches 100 percent at a known size [@d18committee].
...
$\text{Cond3}$ is rarely (if ever) met in physical scenarios because it requires that only a single size be present between any two adjacent sieves.
The final MDM solution will not be constrained by $\text{Cond3}$, but it is helpful for formulating the solution.
...
Specifically, 2750 randomized GSDs based on distributions across the sieve set specified in ASTM D6913 (14 opening sizes from 0.075-mm to 75-mm) are analyzed.
...
@fig-gsd_curvature_index shows a subset (about 20%) of the GSDs considered and illustrates the definition of $I_C$ on an example GSD.
...
#| fig-cap: "A representative subset (~10%) of the GSDs evaluated. The area ratio defining the curvature index is shown. The curve in the highlighted example has a curvature index of 1.09."
...
For any given sample, creating a grain size distributionthat accurately describes it is no more complicated than following the numerical equivalent to a sieve analysis.
The reverse, however, finding a solution for $S$ that is accurately described by some target $G$, is non-trivial.
...
For example, in space-filling solutions, such as presented by @botet2021, particle size is constrained by 3-D geometry rather than the 1-D bounds of a mass distribution-filling problems.
Second, comparing the "minimal" solution sizes from the fixed size and spanned integer algorithms provides a measure of how sensitive the minimal discrete match problem is to the selection of sizes.
...
The results are presented in terms of $\phi_1$, $\zeta_1$, $I_C$, and USCS classification.
...
Curves with high $I_C$ tend to involve larger relative masses at lower sizes than those with low $I_C$, which is conceptually similar to $\phi_1$, but averaged over the entire distribution.
...
ax.set_xlabel("N (MDM predicted)")
ax.set_ylabel("N (reported)")
ax.grid(False)
...
While traditional sieve analyses, and the analogous mathematical representation presented herein do not provide any additional constraints, natural soil grain distributions are known to be broadly distributed between sieves [@ghalib1999].
...
It is important to understand that an MDM does not constitute a representative volume (RV).
An MDM is defined purely in terms of discrete mass-volume relationships.
The RV has additional requirements about the mechanical behavior.
Nevertheless, the mechanical behavior of granular materials is strongly influenced by its GSD, so the discrete mass-volume relationship cannot be fully decoupled from the RV.
In this sense, the MDM can be thought of as a building block and minimum bound on the particle count in RV, which can be useful in assessing computational cost.

The behavior being modeled may allow some portions of realistic GSDs to be truncated without loss of fidelity, allowing a partial decoupling of MDM and RV.
...
Depending on whether the modeling application calls for a rigorous match between a target GSD and its DEM representation, either a rigorous or approximate MEM approach may be appropriate.

The MDM is a valuable tool in quantifying relative computational cost and assessing feasibility of building DEM models of different grain size distributions.
To be used effectively, the representative volume for the modeling application must be considered.
...
The distribution of MDM size across USCS classifications indicates that a wide range of realistic granular soils are feasible for DEM modeling.
