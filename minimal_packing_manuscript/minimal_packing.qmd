...
<!-- Linear dimensions (i.e., radius or diameter) are typically used to describe particle size in geomechanics. -->

$$S = \{(X, Q) \in I_S\}$$ where:

$X_S = \{x_i : i \in I_S\}$ with $x_1 < x_2 < \cdots < x_{n_S}$
...
$$G = \{(X_G, M_G) \in I_G\}$$ where:

$X_G = \{x_j : j \in I_G\}$ with $x_1 < x_2 < \cdots < x_{n_G}$

$M = \{m_j : j \in I_G\}$

$X_G \subset \mathbb{R}^{non-neg}$ and $V \subset \mathbb{R}^{non-neg}$

**Condition 0** (completeness): $$G \text{is complete} \Leftrightarrow v_{n_G} = 0$$
...
**Condition 2:** $G$ describes $S$ accurately if for all consecutive pairs $(x_j, x_{j+1})$ in $X_G$: $$m_j = \sum_{\substack{i \in I_S \\ x_j < x_i \le x_{j+1}}} q_i \cdot f(x_i)$$ where $f(x)$ is a scaling function that converts size to mass.

**Condition 3:** $G$ describes $S$ efficiently if $\forall (x_j, x_{j+1})$ consecutive in $X_G$, $\exists x_i \in X_S$ such that $x_j < x_i \le x_{j+1}$

**Condition 4:** $G$ describes $S$ articulately if $\forall (x_i, x_{i+1})$ consecutive in $X_S$, $\exists x_j \in X_G$ such that $x_i \le x_j < x_{i+1}$
