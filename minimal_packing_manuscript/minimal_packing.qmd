...
Note that $Z$ is referred to as a volume ratio to avoid convusion with $\Phi$ despite the fact that the mapping function $f(x)$ converts size to mass.
...
Rounding each element in $K$ to the nearest integer provides an approximate solution for the minimal discrete match.
...
The effectiveness of the spanned integer algorithm is shown in the rapid convergence to numerical zero error compared to a fixed-size approximation in @fig-convergence. The figure shows both approaches attempting to find a suitable sample for each of the GSDs shown in @fig-gsd_curvature_index with an allowable error tolerance of 1 percent.
In nearly all cases, the spanned integer approach requires no iteration.
...
The MDM increases with increasing mass ratio.
The most critical mass ratio is smallest size to largest size.

The same is true of the volume ratio, which spans a larger range than volume ratio.
...
@fig-curvature_N shows the relationship between I_C and MDM size.
...
@fig-uscs_N shows the range of MDMs associated with the granular uscs classifications in this study.
